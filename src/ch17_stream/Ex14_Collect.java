...
public class Ex14_Collect {
	...
	public static void main(String[] args) {
		...
		totalList.add(new Ex14_Student("김자바", "남", 95));
		totalList.add(new Ex14_Student("오해영", "여", 93));

		// 남학생
		List<Ex14_Student> maleList = totalList.stream().filter(s -> s.getGender().equals("남"))
				.collect(Collectors.toList());
		maleList.forEach(st -> System.out.println(st.getName()));

		maleList = totalList.stream().filter(st -> st.getGender().equals("남")).toList();
		maleList.forEach(st -> System.out.println(st.getName()));
