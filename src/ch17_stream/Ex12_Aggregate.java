...
public class Ex12_Aggregate {
	...
	public static void main(String[] args) {
		...
		// 2의 배수의 최대값
		int max = Arrays.stream(intArr).filter(n -> n % 2 == 0).max() // OptionalXXX type
				.getAsInt();
...
//							.getAsDouble();
		...
		// 해결방법1
		OptionalDouble optional = Arrays.stream(intArr).filter(n -> n % 5 == 0).average();
		...
		// 해결방법2
		double average = Arrays.stream(intArr).filter(n -> n % 5 == 0).average().orElse(0.0);
		System.out.println("방법2 평균: " + average);

		// 해결방법3
		Arrays.stream(intArr).filter(n -> n % 5 == 0).average().ifPresent(d -> System.out.println("방법3 평균: " + d));
	}
