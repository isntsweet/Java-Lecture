...
public class Ex15_Grouping {
	...
	public static void main(String[] args) {
		...
		// 학생 -> 남학생 맵, 여학생 맵
		Map<String, List<Ex15_Student>> map = totalList.stream().collect(Collectors.groupingBy(st -> st.getGender()));
		...
		Map<String, Double> scoreMap = totalList.stream()
				.collect(Collectors.groupingBy(st -> st.getGender(), Collectors.averagingDouble(st -> st.getScore())));
